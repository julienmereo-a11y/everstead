...
// legacy rows written by the previous deploy
ok('legacy plaintext token passes through',    openToken(TOKEN) === TOKEN)
...
ok('legacy plaintext code still matches',      codeMatches('123456', '123456', 'jane@example.com'))
ok('legacy path rejects a wrong code',         !codeMatches('999999', '123456', 'jane@example.com'))
ok('a hash is never treated as legacy',        !codeMatches(h, h.slice(0, 12), 'jane@example.com'))
