...
export default function Dashboard() {
  ...
  const { data: documents,     loading: loadingDocs,
          uploadFile }                                         = useDocuments()
  const { data: people,        loading: loadingPeople,
          invite, resendInvite }                               = usePeople()
  const { data: instructions,  loading: loadingInstructions }  = useInstructions()
  const { data: subscriptions, loading: loadingSubs }          = useSubscriptions()
  ...
  return (
    <div className="min-h-screen bg-stone-50 flex">
      ...
      <main className="flex-1 overflow-auto flex flex-col">
        ...
        {activeSection === 'accounts'      && <AccountsSection  accounts={accounts} loading={loadingAccounts} />}
        {activeSection === 'documents'     && <DocumentsSection documents={documents} loading={loadingDocs} uploadFile={uploadFile} />}
        {activeSection === 'people'        && <PeopleSection    people={people} loading={loadingPeople} invite={invite} resendInvite={resendInvite} />}
        {activeSection === 'instructions'  && <InstructionsSection instructions={instructions} loading={loadingInstructions} />}
        {activeSection === 'subscriptions' && <SubscriptionsSection subscriptions={subscriptions} loading={loadingSubs} />}
...
// ACCOUNTS SECTION
// ─────────────────────────────────────────────────────────────
function AccountsSection({ accounts, loading }) {
  const { add, remove } = useAccounts()
  const [showAdd, setShowAdd] = useState(false)
  const [form, setForm]       = useState({ institution: '', account_type: '', category: 'Banking', account_number_hint: '', balance_display: '', notes: '' })
  ...
  const handleAdd = async (e) => {
    e.preventDefault()
    setSaving(true)
    try { await add(form); setShowAdd(false); setForm({ institution: '', account_type: '', category: 'Banking', account_number_hint: '', balance_display: '', notes: '' }) }
  ...
  return (
    <SectionShell
      ...
      subtitle={`${accounts.length} accounts documented`}
      action={<button onClick={() => setShowAdd(true)} className={primaryBtn}><Plus size={15} />Add account</button>}
    >
        ...
        Object.entries(grouped).map(([category, items]) => {
          ...
          return (
            <div key={category} className="mb-6">
              ...
              <div className="space-y-2">
                {items.map(acc => (
                  <div key={acc.id} className="bg-white border border-stone-200 rounded-xl px-5 py-4 flex items-center gap-4">
                    <div className="flex-1">
                      <p className="font-medium text-navy-900 text-sm">{acc.institution}</p>
                      <p className="text-xs text-stone-500 mt-0.5">{acc.account_type}{acc.account_number_hint ? ` · ••••${acc.account_number_hint}` : ''}</p>
                    </div>
                    {acc.balance_display && (
                      <span className="text-sm font-medium text-navy-800">{acc.balance_display}</span>
                    )}
                    <button onClick={() => remove(acc.id)} className="p-1.5 text-stone-300 hover:text-red-500 transition-colors rounded-lg hover:bg-red-50">
                      <Trash2 size={14} />
                    </button>
                  </div>
                ))}
          ...
          <form onSubmit={handleAdd} className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-4">
              <Field label="Last 4 digits (optional)">
                <input className={input} value={form.account_number_hint} onChange={e => setForm(p => ({...p, account_number_hint: e.target.value}))} maxLength={4} placeholder="4821" />
              </Field>
            ...
            <Field label="Notes">
              <textarea className={input} rows={2} value={form.notes} onChange={e => setForm(p => ({...p, notes: e.target.value}))} placeholder="Any notes…" />
            </Field>
            <div className="flex gap-3 pt-2">
              <button type="submit" disabled={saving} className={`${primaryBtn} flex-1`}>
                {saving ? 'Saving…' : 'Add account'}
              </button>
              <button type="button" onClick={() => setShowAdd(false)} className={secondaryBtn}>Cancel</button>
            </div>
          </form>
        </Modal>
      )}
...
// DOCUMENTS SECTION
// ─────────────────────────────────────────────────────────────
function DocumentsSection({ documents, loading, uploadFile }) {
  const [showUpload, setShowUpload] = useState(false)
  const [file, setFile]             = useState(null)
  const [form, setForm]             = useState({ name: '', doc_type: 'Legal', expires_at: '', notes: '' })
  const [saving, setSaving]         = useState(false)
  ...
  const handleUpload = async (e) => {
    ...
    if (!file) return
    setSaving(true)
    try { await uploadFile(form, file); setShowUpload(false); setFile(null); setForm({ name: '', doc_type: 'Legal', expires_at: '', notes: '' }) }
  ...
  return (
    <SectionShell
      ...
      action={<button onClick={() => setShowUpload(true)} className={primaryBtn}><Upload size={15} />Upload document</button>}
    >
                ...
                <tr key={doc.id} className="hover:bg-stone-50 transition-colors">
                  <td className="px-5 py-3.5">
                    <div className="flex items-center gap-2.5">
                      ...
                      <span className="font-medium text-navy-800">{doc.name}</span>
                    </div>
                  </td>
                  ...
                  <td className="px-5 py-3.5">
                    <div className="flex items-center gap-1.5">
                      {doc.storage_path && (
                        <button className="p-1.5 text-stone-300 hover:text-navy-600 transition-colors rounded hover:bg-navy-50">
                          <Eye size={14} />
                        </button>
                      )}
                      <button onClick={() => remove(doc.id)} className="p-1.5 text-stone-300 hover:text-red-500 transition-colors rounded hover:bg-red-50">
                        <Trash2 size={14} />
                      </button>
                    </div>
                  </td>
        ...
        <Modal title="Upload document" onClose={() => setShowUpload(false)}>
          <form onSubmit={handleUpload} className="space-y-4">
            ...
            <div className="flex gap-3 pt-2">
              ...
              <button type="button" onClick={() => setShowUpload(false)} className={secondaryBtn}>Cancel</button>
            </div>
          </form>
        </Modal>
      )}
        ...
        <Modal title="Invite trusted person" onClose={() => setShowInvite(false)}>
          <form onSubmit={handleInvite} className="space-y-4">
            ...
            <Field label="Role" required>
              <input className={input} value={form.role} onChange={e => setForm(p => ({...p, role: e.target.value}))} placeholder="e.g. Primary Executor, Healthcare Proxy" required />
            </Field>
            <div className="flex gap-3 pt-2">
              <button type="submit" disabled={saving} className={`${primaryBtn} flex-1`}>
...
// ─────────────────────────────────────────────────────────────
function InstructionsSection({ instructions, loading }) {
  return (
    <SectionShell title="Instructions" subtitle={`${instructions.length} instruction sets`} action={<button className={primaryBtn}><Plus size={15} />Add instructions</button>}>
...
// ─────────────────────────────────────────────────────────────
function SubscriptionsSection({ subscriptions, loading }) {
  const total = subscriptions.reduce((sum, s) => sum + (s.billing_cycle === 'Annual' ? s.amount / 12 : s.amount), 0)
  return (
    <SectionShell title="Subscriptions" subtitle={`£${total.toFixed(2)}/mo total`} action={<button className={primaryBtn}><Plus size={15} />Add subscription</button>}>
        ...
        <div className="bg-white border border-stone-200 rounded-xl divide-y divide-stone-50">
          {subscriptions.map(sub => (
            <div key={sub.id} className="flex items-center gap-4 px-5 py-4">
              ...
              <p className="font-semibold text-navy-900 text-sm">£{sub.amount?.toFixed(2)}</p>
            </div>
          ))}
        </div>
      )}
