  ...
  return (
    ...
    <div className="min-h-screen bg-stone-50 flex flex-col">
      ...
      <main ref={mainRef} className="flex-1 overflow-auto flex flex-col min-w-0 pb-24" style={{ backgroundColor: '#f8f7f5' }} aria-label={t('shell.mainContent')}>
        ...
        {activeSection === 'overview'      && <OverviewSection  isDemo={isDemo} adviser={adviserLink} profile={activeProfile} accounts={accounts} documents={documents} people={people} instructions={instructions} messages={messages} alerts={alerts} markRead={markRead} onNavigate={setActiveSection} planLimits={planLimits} loading={loadingAccounts || loadingDocs} daysSinceLogin={daysSinceLogin} onCelebrate={celebrate} onExecutorPreview={() => setShowExecutorPreview(true)} aboutMe={aboutMe} onUpgrade={() => handleUpgrade('family', 'yearly')} persistScore={isDemo ? undefined : updateProfile} scoreInputsLoaded={!loadingAccounts && !loadingDocs && !loadingPeople && !loadingInstructions} />}
  ...
  return (
    <>
      ...
      <button
        ...
        style={{ background: '#2d5082', borderRadius: '24px', border: 'none', boxShadow: '0 8px 20px -10px rgba(13, 22, 40, 0.45)', cursor: 'pointer' }}
        onMouseEnter={e => { e.currentTarget.style.transform = 'translateY(-2px)'; e.currentTarget.style.boxShadow = '0 12px 26px -10px rgba(13, 22, 40, 0.55)' }}
        onMouseLeave={e => { e.currentTarget.style.transform = 'translateY(0)'; e.currentTarget.style.boxShadow = '0 8px 20px -10px rgba(13, 22, 40, 0.45)' }}
