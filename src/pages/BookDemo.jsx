...
export default function BookDemo() {
  ...
  const fromAdvisorLarge = searchParams.get('reason') === 'advisor-large'
  const [form, setForm] = useState({ name: '', email: '', firm: '', role: '', clients: fromAdvisorLarge ? '5+' : '', notes: '' })
        ...
        <div className="max-w-6xl mx-auto px-6 lg:px-8 grid lg:grid-cols-[0.9fr_1.1fr] gap-16">
          {fromAdvisorLarge && (
            <div className="lg:col-span-2 reveal">
              <div className="flex items-start gap-4 bg-navy-50 border border-navy-200 rounded-2xl px-6 py-5">
                <div className="w-9 h-9 rounded-xl bg-navy-800 text-white flex items-center justify-center shrink-0">
                  <Briefcase size={16} />
                </div>
                <div>
                  <p className="text-sm font-semibold text-navy-900">You're managing more than 5 families — let's set you up properly.</p>
                  <p className="mt-1 text-sm leading-relaxed text-stone-500">
                    The self-service plan covers up to 5 client families. For larger practices, our team will tailor the rollout, onboarding, and pricing to your firm. Fill in the form and we'll be in touch within one business day.
                  </p>
                </div>
              </div>
            </div>
            ...
            {sent ? (
              ...
              <form onSubmit={handleSubmit} className="rounded-[2rem] border border-stone-200 bg-white p-8 space-y-4">
                ...
                <div className="grid sm:grid-cols-2 gap-4">
                  ...
                  <Field label="Role">
                    <input name="role" value={form.role} onChange={handleChange} required placeholder="Advisor, solicitor, partner…" className={inputClass} />
                  </Field>
                </div>
