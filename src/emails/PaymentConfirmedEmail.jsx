import {
  Html, Head, Body, Container, Text, Link, Hr, Preview,
} from '@react-email/components'

export default function PaymentConfirmedEmail({ firstName = 'there', plan = 'Essential', billingCycle = 'yearly', amount = '£60' }) {
  return (
    <Html>
      <Head />
      <Preview>You're now on Everstead {plan}, {firstName}.</Preview>
      <Body style={body}>
        <Container style={container}>

          <Text style={logo}>Everstead</Text>

          <Text style={heading}>
            You're all set, {firstName}.
          </Text>

          <Text style={paragraph}>
            Your Everstead <strong>{plan}</strong> plan is now active, billed {billingCycle} at <strong>{amount}</strong>.
          </Text>

          <Text style={paragraph}>
            Your family now has a plan in place. That matters more than most people realise — until they need it.
          </Text>

          <Hr style={hr} />

          <Text style={paragraph}>A few things you can do next:</Text>

          <Text style={listItem}>→ Add your key accounts and documents</Text>
          <Text style={listItem}>→ Invite a trusted person to your plan</Text>
          <Text style={listItem}>→ Write your step-by-step instructions</Text>

          <Hr style={hr} />

          <Link href="https://www.everstead.care/dashboard" style={button}>
            Go to your plan →
          </Link>

          <Text style={paragraph}>
            If you have any questions about your subscription, just reply to this email.
          </Text>

          <Text style={signature}>
            Julien<br />
            Founder, Everstead
          </Text>

          <Hr style={hr} />
          <Text style={footer}>
            © 2026 Everstead Digital Ltd · Registered in England & Wales
          </Text>

        </Container>
      </Body>
    </Html>
  )
}

const body = { backgroundColor: '#f8f7f5', fontFamily: 'Georgia, serif' }
const container = { margin: '0 auto', padding: '40px 24px', maxWidth: '560px' }
const logo = { fontSize: '22px', fontWeight: '400', color: '#0d1628', letterSpacing: '-0.02em', marginBottom: '32px' }
const heading = { fontSize: '24px', fontWeight: '400', color: '#0d1628', lineHeight: '1.3', marginBottom: '24px' }
const paragraph = { fontSize: '15px', color: '#44403c', lineHeight: '1.7', marginBottom: '16px' }
const listItem = { fontSize: '15px', color: '#44403c', lineHeight: '1.7', marginBottom: '8px', paddingLeft: '8px' }
const button = { display: 'inline-block', backgroundColor: '#0d1628', color: '#ffffff', fontSize: '14px', fontWeight: '600', padding: '12px 24px', borderRadius: '8px', textDecoration: 'none', marginBottom: '24px' }
const signature = { fontSize: '15px', color: '#0d1628', lineHeight: '1.7', marginTop: '24px' }
const hr = { borderColor: '#e5e2dc', margin: '24px 0' }
const footer = { fontSize: '11px', color: '#a8a29e', textAlign: 'center' }