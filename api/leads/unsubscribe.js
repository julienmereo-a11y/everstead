...
// One-click unsubscribe for marketing leads (free-tool email captures).
// Token is a 32-char hex string stored on the lead row.
...
async function handler(req, res) {
  ...
  if (!data || data.length === 0) {
    return res.status(200).send(page("You're unsubscribed", "This email is no longer on the marketing list."))
  }

  return res.status(200).send(page("You're unsubscribed", `${data[0].email} has been removed from the Everstead marketing list.`))
}
...
<head>
  ...
  <style>
    ...
    a { display:inline-block; background:#2d5082; color:#fff; padding:10px 22px; border-radius:9999px; text-decoration:none; font-weight:500; font-size:14px; }
...
<body>
  <div class="card">
    <h1>${title}</h1>
    <p>${message}</p>
    <a href="https://www.everstead.care">Back to Everstead</a>
  </div>
</body>
</html>`
}
